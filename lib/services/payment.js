  ...
  try {
    ...
    const response = await fetch(COINBASE_CHARGE_URL, {
      ...
      body: JSON.stringify({
        ...
        description: chargeData.description,
        pricing_type: chargeData.pricing_type || 'fixed_price',
        local_price: {
    ...
    if (!response.ok) {
      throw new Error(chargeResponse.error?.message || 'Failed to create charge');
    }
    ...
    const { error: cryptoError } = await supabase
      ...
      .insert({
        payment_id: paymentRecord.id,
        cryptocurrency: 'pending', // Will be updated when user selects
        wallet_address: chargeResponse.data.addresses?.bitcoin || '',
        transaction_hash: null // Will be updated when payment is confirmed
      });

    if (cryptoError) console.error('Error storing crypto details:', cryptoError);
